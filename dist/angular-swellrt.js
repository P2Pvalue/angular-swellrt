...
angular.module('SwellRTService', []).factory('swellRT', ['$rootScope', '$q', '$timeout', '$browser', function ($rootScope, $q, $timeout, $browser) {
  ...
  function addParticipantsObservableList(model, proxy) {
    ...
    model.registerEventHandler(SwellRT.events.PARTICIPANT_ADDED, function (participant) {
      proxy._participants.push(participant);
      $timeout();
    });

    model.registerEventHandler(SwellRT.events.PARTICIPANT_REMOVED, function (participant) {
  ...
  var reservedRootKeys = ['_id', '_participants'];
  ...
  function registerEventHandlers(e, m, p, model) {
    ...
    depthFirstFunct(e, m, p, function (elem, mod, path) {
      elem.registerEventHandler(SwellRT.events.ITEM_CHANGED, function (newStr) {
        try {
      ...
      elem.registerEventHandler(SwellRT.events.ITEM_ADDED, function (item) {
      ...
      elem.registerEventHandler(SwellRT.events.ITEM_REMOVED, function (item) {
        var par = path.reduce(function (object, key) {
          return object[key];
        }, mod);
        // unwatch
        unwatchMap[path.join()][0]();
    ...
    // FileType
    function (elem, mod, path) {
      elem.registerEventHandler(SwellRT.events.ITEM_CHANGED, function (newValue) {
    ...
    depthFirstFunct(e, m, p, function (elem, mod, path) {
      ...
      function watch() {
        ...
        var unwatch = $rootScope.$watchCollection(function () {
          ...
          if (path.join() === '') {
            oldKeys.push.apply(oldKeys, reservedRootKeys);
          }
      ...
      var unwatch = watch();
      unwatchMap[path.join()] = [unwatch, watch];
    }, undefined, function (elem, mod, path) {
      function watch() {
        var unwatch = $rootScope.$watch(function () {
          try {
            var r = path.reduce(function (object, key) {
  ...
  function join(emailOrUser, waveUrl, waveTitle, text, user, onSuccess, onFailure) {
    ...
    SwellRT.join(emailOrUser, waveUrl, waveTitle, text, user, function (result) {
    ...
    ngModelCtrl.$formatters.unshift(function (modelValue) {
      if (modelValue) {
        ...
        if (ph) {
          ...
          style.type = 'text/css';
          style.innerHTML = '.swellrt-placeholder:empty:before {' + 'content: attr(placeholder)' + '}';
          document.getElementsByTagName('head')[0].appendChild(style);

          var editorDiv = element.children();

          var editorDivs = angular.element(element.children().children().children()[0]);

          editorDivs.addClass('swellrt-placeholder').attr('placeholder', ph);
          var emptyPad = function emptyPad() {
            return editorDiv.children().children().length === 1 && (editorDivs.html() === '<br>' || editorDivs.html() === '');
          };
          ...
          editorDiv.attr('tabindex', '-1').on('blur', function () {
            if (emptyPad()) {
              editorDivs.empty();
            }
          });
  ...
  function link(scope, element) {
    ...
    scope.$watch('ngModel', function (value) {
      ...
      if (scope.swellrtAvatarOptions) {
        scope.swellrtAvatarOptions.numberOfAvatars = Math.min(avatarAttrs.length, scope.swellrtAvatarOptions.numberOfAvatars);
      }
